# S3 Endpoint 설정: VPC 내에서 서비스에 대한 프라이빗 액세스를 제공하는 Gateway Endpoint 생성
...
resource "aws_vpc_endpoint" "s3_endpoint" {
  ...
  # for rt in aws_route_table.std07_private_rt : rt.id
  route_table_ids = concat(
    [for rt in aws_route_table.private_rt : rt.id],
    [aws_route_table.cluster_rt.id]
  )
  tags = { Name = "${local.tag_header}-s3-endpoint" }
}

# ECR 서비스 사용울 위한 InterfaceInspoint
# - ECR API Interface Endpoint:
#   IAM인증, 이미지 메타데이터 조회, 레포지토리 생성 및 삭제 등 ECR API 제어 명령을 처리
